...
public final class QueryUtils {
    ...
    /**
     * Sample JSON response for a USGS query,this sample response is also copies in its original form in D:\Android Topics\SAMPLE_JSON_RESPONSE in this pc
     */
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //converting the sampole JSON Response string to json object
            JSONObject jsonString = new JSONObject(SAMPLE_JSON_RESPONSE);

            //extracting eatures JSON array
            JSONArray features = jsonString.getJSONArray("features");
            ...
            for (int i = 0; i < features.length(); i++) {
                ...
                //-> long timeInMilli will fetch time in milliSeconds but we wnt the time in the proper date format such as  YYYY-DD-MM,
                //so android comes up with a fantastic class -> SimpleDateFormat("MMM DD, yyyy") class in which we pass the format as the argument
                //SimpleDateFormat() class takes in a date object.
//                Here’s an example for the time 1463159138711 milliseconds.
//                We create a SimpleDateFormat object with the format "yyyy-MM-dd HH:mm:ss a",
//                and pass in a Date object initialized with the time 1463159138711 milliseconds, and then the date will be formatted as “2016-05-13 12:07:46 PM”.

                //creating the date object first
                Date dateObject = new Date(timeInMilli);

                // making an object of SimpleDateFormat class
                SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
                String time = dateFormatter.format(dateObject);

                //Passing all the three values in Earthquake constructor
